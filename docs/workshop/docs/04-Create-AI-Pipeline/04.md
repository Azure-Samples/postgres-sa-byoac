...
The SOW and Invoice validation step of the document ingestion workflow uses Azure OpenAI in Azure AI Foundry to perform a chat completion using retrieval augmented generation (RAG) pattern to validate the data from the database and generate an analysis of the validation result.
