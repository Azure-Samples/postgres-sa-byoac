...
You will use the **PostgreSQL extension in VS Code** to install the extensions by executing SQL commands against your database.

1. On your local machine, return to the open instance of **VS Code** (or open it if you closed it after the setup tasks), select the PostgreSQL extension icon (elephant) from the Activity Bar, and ensure it is connected to your PostgreSQL database.
...
!!! danger "Select each of the tabs below and execute the `CREATE EXTENSION` command in the PostgreSQL extension's query window to install the extensions."

    Execute queries in the PostgreSQL query window by selecting the **Execute PostgreSQL Query** button (green run icon) at the top right of the query window, or by pressing `SHIFT+ENTER` on your keyboard.
