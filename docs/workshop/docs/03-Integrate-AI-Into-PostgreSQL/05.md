...
```sql title="Function signatures for the create_embeddings() function" linenums="0"
-- Single text input
...
To demonstrate how to generate vector embeddings through a SQL query, execute the following query in **PostgreSQL in VS Code**.

1. Return to the open instance of **VS Code** on your local machine, select the PostgreSQL extension icon in the Activity Bar, and ensure it is connected to your PostgreSQL database.
...
3. Run the following query, which creates a vector embedding for the `result` field in the `invoice_validation_results` table. The `deployment_name` parameter in the function is set to `embeddings`, which is the name of the deployment of the `text-embedding-ada-002` model in your Azure OpenAI service (it was created with that name by the Bicep deployment script):
    ...
    ```sql title="Sample query to generate embeddings" linenums="0"
    SELECT 
        invoice_id,
...
4. The results of the query will look similar to this:

    ```sql title="" linenums="0"
       invoice_id |                              embedding
    --------------+-------------------------------------------------------------------------
...
Using **PostgreSQL in VS Code**, execute the SQL statement for each table.
...
=== "deliverables"
    ...
    Copy and paste the following SQL statement into a new query window in PostgreSQL in VS Code, then execute the query.
...
!!! info "Generate Embeddings on Database `INSERT` using a Trigger"
    ...
    ```sql title="Example INSERT Trigger to Generate Embeddings" linenums="0"
