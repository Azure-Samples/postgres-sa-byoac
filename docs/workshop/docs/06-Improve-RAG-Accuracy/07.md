...
You will execute openCypher queries using **PostgreSQL in VS Code** to verify the data load and explore relationships in your graph database.

1. Return to **PostgreSQL in VS Code** and ensure it is connected to your PostgreSQL database.

2. Right-click the **contracts** database and select **New Query** from the context menu.

3. Before you can run cypher queries, you must set the `ag_catalog` schema in your path:
...
4. Now, run the following cypher query to view vendors with open invoices, the details of those invoices, and verify your graph database was loaded correctly:
