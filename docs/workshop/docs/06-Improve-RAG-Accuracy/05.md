...
You will use **PostgreSQL in VS Code** to install the extensions by executing SQL commands against your database.
...
3. Install the `age` and `azure_storage` extensions by running the following `CREATE EXTENSION` commands in the PostgreSQL query window:
