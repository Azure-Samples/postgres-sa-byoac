...
You will use **PostgreSQL in VS Code** to configure the `azure_storage` extension's connection to your storage account by executing SQL commands against your database.
...
1. On your local machine, return to the open instance of **PostgreSQL in VS Code**.

2. Right-click the **contracts** database and select **New Query** from the context menu.

3. Run the following command to use the `azure_storage.account_add()` function to define a connection between your storage account and your PostgreSQL database. Replace the account name and key tokens with values for your storage account.

    !!! danger "Execute the following SQL commands in PostgreSQL in VS Code!"
...
You will use **PostgreSQL in VS Code** to execute data export queries leveraging the `azure_storage` extension.

1. Return to the open **Query** in **PostgreSQL in VS Code**.
...
1. Execute the following query in the Query window in **VS Code**. Ensure you replace the `[YOUR_STORAGE_ACCOUNT_NAME]` token with the name of your storage account.
...
2. You should see a list of blobs in the **PostgreSQL Query Results** panel in **VS Code** that includes the three CSV files you exported above.
